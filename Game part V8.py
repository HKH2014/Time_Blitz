...
""")
        ...
        print("Error: " , wrong_value , "Please restart program")
        make_account()
...
def next_steps():
    ...
    level = input("What level? Enter KS1, KS2 OR KS3")
    ...
    # checking that player is an authenticated player by reading the lines in the external CSV file
        ...
        with open ("UserDetails.csv", "r" , newline="") as UserInfo:
            ...
            for row in person:
                personA_name = row[0]
                personA_pass = row[1]
                StoreUsername.append(personA_name)
                StoreUserpass.append(personA_pass)
        UserInfo.close()
            
        if username in StoreUsername:
            if password in StoreUserpass:# 
